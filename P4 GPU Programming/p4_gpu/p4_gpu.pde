...
void draw() {
  ...
  float d = 5;
  for (int x = -300; x < -50; x+= d) {
    for (float y = 50; y < 300-d; y+= d) {
    
      float sMin = 1- (x + 50.0) / -250.0;
      float sMax = 1- (x + d + 50.0) / -250.0;
      float tMin = (y - 50.0) / 250.0;
      float tMax = (y + d - 50.0) / 250.0;
      
      shader(bumps_shader);
      beginShape();
      texture(bumps_texture);
      vertex(x,   y,   -300, sMin, tMin);
      vertex(x+d, y,   -300, sMax, tMin);
      vertex(x+d, y+d, -300, sMax, tMax);
      vertex(x,   y+d, -300, sMin, tMax);
      endShape();
    }
  }
